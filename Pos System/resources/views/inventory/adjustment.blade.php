<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">⚖ Stock Adjustment</h2>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl p-6">
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-5">Set the exact stock count for a product (e.g., after a physical stock count).</p>

                <form action="{{ route('inventory.adjustment.store') }}" method="POST" class="space-y-5">
                    @csrf
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Actual Quantity (New Count) <span class="text-red-500">*</span></label>
                        <input type="number" name="new_quantity" id="new_quantity" value="{{ old('new_quantity') }}" min="0" required
                               class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                        <p id="diffDisplay" class="text-xs mt-1 font-medium hidden"></p>
                        @error('new_quantity') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Notes / Reason <span class="text-red-500">*</span></label>
                        <textarea name="notes" rows="2" required
                                  class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                                  placeholder="e.g. Physical stock count on 13 June 2024">{{ old('notes') }}</textarea>
                    </div>
    ...
    <script>
        // Removed dynamic stock update since stock varies by showroom
